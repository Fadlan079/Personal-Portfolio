...
document.addEventListener('DOMContentLoaded', () => {
    ...
    layoutBtn?.addEventListener('click', () => {
        ...
        if (csrfToken) {
            ...
            }).then(() => { if(window.triggerPageWipe) window.triggerPageWipe(window.location.href, `Mengganti tema UI...`); else window.location.reload(); })
              .catch(() => { if(window.triggerPageWipe) window.triggerPageWipe(window.location.href, `Mengganti tema UI...`); else window.location.reload(); });
        } else {
            if(window.triggerPageWipe) window.triggerPageWipe(window.location.href, `Mengganti tema UI...`); else window.location.reload();
        }
    });
    ...
    langBtn?.addEventListener('click', () => {
        ...
        if (csrfToken) {
            fetch('/api/locale', {
                ...
                if(window.triggerPageWipe) window.triggerPageWipe(window.location.href, `Mengganti bahasa ke ${nextLocale.toUpperCase()}...`); else setTimeout(() => { window.location.reload(); }, 300);
            }).catch(err => {
