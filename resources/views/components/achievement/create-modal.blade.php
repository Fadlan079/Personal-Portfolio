...
<div id="createAchievementModal" class="fixed inset-0 z-70 hidden items-center justify-center bg-stone-900/60 backdrop-blur-sm p-4 md:p-6">
    ...
    <div class="relative w-full max-w-3xl max-h-[85vh] overflow-y-auto hide-scrollbar bg-[#FCFAEF] text-stone-800 shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-stone-200/60 rounded-sm">
        ...
        <button id="closeCreateAchievementModal" type="button" class="absolute top-6 right-6 text-stone-400 hover:text-stone-800 transition-colors z-10 text-xl font-light">
            ✕
        </button>
        ...
        <form action="{{ route('dashboard.achievements.store') }}" method="POST" enctype="multipart/form-data" class="px-8 pb-8 md:px-12 space-y-8">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative border border-dashed border-stone-400 bg-white/30 rounded-sm p-6 text-center hover:bg-white/60 hover:border-stone-800 transition group cursor-pointer">
                        ...
                        <div id="placeholderCreateAchievement" class="space-y-2 pointer-events-none">
                            <div class="text-stone-400 group-hover:text-stone-800 text-2xl transition-colors font-light">＋</div>
                            <p class="font-diary-body text-xs text-stone-500 group-hover:text-stone-800 transition-colors">Clip image here...</p>
                            <p class="text-[10px] text-stone-400 uppercase tracking-widest">PNG, JPG, JPEG (Max. 10MB)</p>
                        </div>

                        <div id="previewCreateAchievementContainer" class="hidden relative w-full aspect-video bg-stone-100 rounded-sm overflow-hidden z-10 border border-stone-200">
                            <img id="previewCreateAchievement" src="#" alt="Preview" class="w-full h-full object-contain filter contrast-[0.95] sepia-[0.1]">
                            <div class="absolute inset-x-0 bottom-0 bg-stone-900/60 text-white text-xs py-1 px-3 backdrop-blur-sm justify-between items-center hidden group-hover:flex">
                                <span id="fileNameCreateAchievement" class="truncate line-clamp-1 inline-block text-left w-3/4"></span>
            ...
            <div class="flex justify-end gap-4 pt-6 md:pt-8 border-t border-stone-300/60 mt-8">
                <button type="button" id="cancelCreateAchievement" class="px-6 py-2.5 bg-transparent border border-stone-400 text-stone-600 font-diary-body font-bold text-sm hover:border-stone-800 hover:text-stone-800 transition-colors rounded-sm">
                    Batal
                </button>
...
<script>
    function previewAchievementImage(event, previewId) {
        const file = event.target.files[0];
        const container = document.getElementById(previewId + 'Container');
        const placeholder = document.getElementById('placeholder' + previewId.replace('preview', ''));
        const preview = document.getElementById(previewId);
        const fileName = document.getElementById('fileName' + previewId.replace('preview', ''));
