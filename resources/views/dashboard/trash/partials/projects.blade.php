    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between border-b-2 border-dashed border-border/60 pb-3 relative">
            ...
            <h2 class="text-2xl font-serif font-bold text-text pl-6 flex items-center gap-3">
                {{ $month }}
                <span class="text-xs font-sans px-2 py-0.5 bg-border/20 text-muted rounded-full">{{ count($projects) }} Item</span>
            </h2>
        ...
        <div class="grid grid-cols-1 gap-4">
            @foreach ($projects as $index => $project)
                ...
                <div class="project-card border-2 border-border bg-[#FCFAEF] p-5 md:p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 transition-all hover:-translate-y-1 shadow-[4px_4px_0px_var(--color-border)] rounded-sm {{ $rotation }} hover:rotate-0"
                    data-month="{{ Str::slug($month) }}">
                    ...
                    <div class="flex items-start md:items-center gap-5 w-full md:w-auto">
                        <div class="pt-1 md:pt-0 relative">
                            <input type="checkbox"
                                name="projects[]"
                                value="{{ $project->id }}"
                                class="bulk-checkbox w-5 h-5 border-2 border-border rounded appearance-none checked:bg-primary checked:border-primary transition-colors cursor-pointer shadow-sm opacity-0 pointer-events-none">
                        </div>

                        <div class="flex-1">
                            <h3 class="text-lg font-bold font-serif text-neutral-800 mb-1 line-clamp-1">
                                {{ $project->title }}
                            </h3>
                            <p class="text-xs font-mono text-muted mb-2">
                                <i class="fa-regular fa-clock mr-1"></i> Dihapus: {{ $project->deleted_at->format('d M Y - H:i') }}
                            </p>
                            ...
                            <div class="inline-flex items-center gap-2 px-2.5 py-1 rounded bg-surface border border-border text-[10px] uppercase font-bold tracking-wider shadow-inner">
                                @if($daysLeft > 0)
                                    ...
                                    @else
                                        ...
                                        <span class="text-muted">Hapus permanen dalam {{ intval($daysLeft) }} hari</span>
                                    @endif
                                @else
                                    <i class="fa-solid fa-fire text-red-500"></i>
                                    <span class="text-red-600 font-black">Dijadwalkan untuk dihapus permanen</span>
                                @endif
                            </div>
                        </div>
                    </div>

                    <div class="flex items-center gap-3 w-full md:w-auto pt-4 md:pt-0 border-t border-dashed border-border/50 md:border-0 mt-2 md:mt-0 normal-actions">
                        <button type="button"
                            data-restore-url="{{ route('dashboard.projects.restore', $project->id) }}"
        ...
        @if (!$multipleSelect && $projects instanceof \Illuminate\Pagination\LengthAwarePaginator && $projects->hasPages())
    ...
    <div class="col-span-full py-20 px-6 flex flex-col items-center justify-center text-center bg-[#fdfcf5] border-2 border-dashed border-[#e5e0d0] rounded-xl shadow-[4px_4px_0px_rgba(0,0,0,0.05)] relative overflow-hidden mt-8">
        ...
        <div class="relative z-20 space-y-4">
            ...
            <p class="text-sm text-muted max-w-sm mx-auto italic opacity-90">
                Tidak ada proyek yang sedang dalam masa penghapusan saat ini.
            </p>
            <a href="{{ route('dashboard.projects.index') }}" class="inline-block mt-4 px-6 py-2 bg-surface border-2 border-border rounded-lg text-xs font-bold uppercase tracking-widest text-text hover:border-primary transition-colors shadow-sm">
                Kembali ke Beranda
            </a>
        </div>
    </div>
